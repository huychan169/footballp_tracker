...
try:
    ...
    raise ImportError(
        "TorchreIDInterface requires the `torchreid` package. "
        "Install it via `pip install git+https://github.com/KaiyangZhou/deep-person-reid.git`."
    ) from exc
...
class TorchreIDInterface:
    """
    Lightweight wrapper inspired by football/2d_heatmap/BoT-SORT that loads a
    torchreid backbone and exposes an inference function BoT-SORT can consume.
    """
    ...
    @torch.no_grad()
    def inference(self, image: np.ndarray, dets_xyxy: np.ndarray):
